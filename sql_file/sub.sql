...
insert into exam
(id, hakgi, name, pid, reg_date, kor, eng, mat) values
((select max(id)+1 from exam ex), 1, 'final', (select id from mem2 where name = '짠백이'), now(),91,92,98);
...
select * from professor 
where pid = (select prof from mem2 where id = 
(select pid from
(select *, mid('가양미우수',interval(avg, 0, 60,70,80,90),1) as grade from 
--  가상의 테이블
(select *, tot / 3 as avg from
...
select * from exam
where mat > (select mat from exam ex
where pid = 'fff');
