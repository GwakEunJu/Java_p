...
select * from mem2 where name like '이';
select * from mem2 where name like '%이';	-- '이'로 끝난 모든 글자
select * from mem2 where name like '%이%';  -- '이'를 포함 하는 글자
...
select concat(name, '안냐세여') from mem2;
...
select * from exam
where hakgi = 1 -- where이 order by 보다 먼저 들어가야한다 
order by name desc, pid;
