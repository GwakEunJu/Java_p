...
public class GenericArrayListMain {
	public static void main(String[] args) {
		...
		ArrayList<FootPhone> fhArr = new ArrayList();
		...
		// fhArr.add(new TV());
		// fhArr.add(new Phone());
		// fhArr.add(new HandPhone());
		fhArr.add(new FootPhone()); // 자신만 가능(부모 불가)
		System.out.println("fhArr : " + fhArr);
		...
		// ehArr.add(new FootPhone()); 모두 불가- 의미없음
...
//		methPP(objArr);	//Generic 이 설정된 메소드에서는 Object ArrayList 는 하지 말것
		methPP(ppArr);	//자신 + Object 호출 가능한 것으로 보이나 내부 코드에서는 Object ArrayList 에러발생
//		methPP(hhArr);
//		methPP(fhArr);
//		methPP(shArr);
//		methPP(ehArr);
